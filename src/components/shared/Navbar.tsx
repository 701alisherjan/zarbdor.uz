        ...
        <div className="hidden lg:flex flex-1 justify-center items-center gap-6">
          {!searchActive ? (
            <div className="flex flex-wrap justify-center gap-6 items-center">
              ...
              {["USD", "EUR", "RUB"].map((ccy) => (
                <span key={ccy} className="transition-all duration-500 text-sm sm:text-base">
                  {ccy}{" "}
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Button size="icon" variant="ghost" onClick={() => setSearchActive(false)}>
                <X size={20} />
              </Button>
            </div>
          )}
      ...
      {["USD", "EUR", "RUB"].map((ccy) => (
        <div key={ccy} className="flex gap-1 items-center text-sm">
          ...
          <b className="text-gray-900 dark:text-gray-100">
            {rates[ccy] || (ccy === "USD" ? "11 972.35" : ccy === "EUR" ? "13 852.01" : "147.52")}
          </b>
          <span className={`${showAlt ? "text-red-500" : "text-green-500"} font-semibold`}>
            {showAlt ? "-17.65" : "+12.44"}
          </span>
        </div>
      ))}
